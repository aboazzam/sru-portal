...
export default async function GraduatesPage() {
  ...
  const nextDeadline = graduationTimeline.find((t) => !t.done);

  const statusConfig = {
    completed:   { label: "مكتمل",    color: "#16A34A", bg: "#DCFCE7" },
    in_progress: { label: "جارٍ",     color: "#D97706", bg: "#FEF3C7" },
    pending:     { label: "معلّق",    color: "#DC2626", bg: "#FEE2E2" },
    waived:      { label: "مُعفى",    color: "#6B7280", bg: "#F3F4F6" },
  };
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        <div className="mt-5">
          <div className="flex justify-between text-xs text-white/80 mb-1.5">
            <span>اكتمال متطلبات التخرج</span>
            <span>{completed} / {total} ({overallPct}%)</span>
          </div>
          ...
          {overallPct < 100 && (
            <div className="mt-2 flex items-center justify-between">
              <p className="text-white/60 text-xs">أكمل المتطلبات المتبقية للتأهل للتخرج</p>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: "متطلبات مكتملة",  value: `${completed}/${total}`, icon: "✅", color: "border-green-200  bg-green-50  text-green-700"  },
          { label: "نسبة الاستعداد",  value: `${overallPct}%`,        icon: "📊", color: "border-rose-200   bg-rose-50   text-rose-700"   },
          { label: "طلبات المنح",     value: schApps,                  icon: "🎓", color: "border-purple-200 bg-purple-50 text-purple-700" },
          { label: "برامج التدريب",   value: trainingEnrollments,      icon: "🏢", color: "border-blue-200   bg-blue-50   text-blue-700"   },
        ].map((s) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span>📋</span>
              <h2 className="font-bold text-gray-800 text-sm">متطلبات التخرج</h2>
            </div>
            <Link href="/graduates/requirements" className="text-xs text-rose-600 hover:underline font-medium">عرض الكل</Link>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span>🗓️</span>
              <h2 className="font-bold text-gray-800 text-sm">الجدول الزمني للتخرج</h2>
            </div>
            {nextDeadline && (
              <span className="text-xs font-medium text-rose-600 bg-rose-50 px-2.5 py-0.5 rounded-full border border-rose-200">
                الموعد القادم: {new Date(nextDeadline.date).toLocaleDateString("ar-SA", { day: "numeric", month: "short" })}
              </span>
            )}
          </div>
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <h2 className="font-bold text-lg">حفل التخرج</h2>
            </div>
            ...
            <p className="text-rose-300 text-xs mt-0.5">
              آخر موعد للتسجيل: {new Date(ceremonyDetails.registrationDeadline).toLocaleDateString("ar-SA", { day: "numeric", month: "long" })}
            </p>
          </div>
          <Link
          ...
          >
            {ceremonyDetails.registered ? "✅ مسجّل" : "سجّل الآن"}
          </Link>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        {[
          { href: "/graduates/requirements", icon: "✅", label: "متطلبات التخرج", color: "bg-green-50  border-green-200  text-green-700"  },
          { href: "/graduates/ceremony",     icon: "🎓", label: "حفل التخرج",     color: "bg-rose-50   border-rose-200   text-rose-700"   },
          { href: "/graduates/documents",    icon: "📄", label: "وثائق التخرج",   color: "bg-blue-50   border-blue-200   text-blue-700"   },
          { href: "/alumni",                 icon: "🤝", label: "بوابة الخريجين", color: "bg-purple-50 border-purple-200 text-purple-700" },
          { href: "/alumni/jobs",            icon: "💼", label: "فرص العمل",      color: "bg-amber-50  border-amber-200  text-amber-700"  },
          { href: "/alumni/card",            icon: "🪪", label: "بطاقة الخريج",  color: "bg-teal-50   border-teal-200   text-teal-700"   },
        ].map((link) => (
          <Link
            key={link.href}
            href={link.href}
