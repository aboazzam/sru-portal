...
const moduleLinks = [
  { href: "/financial",    icon: "💳", label: "الخدمات المالية",  primary: "#0E7490", bg: "#ECFEFF" },
...
export default async function StudentOverview() {
  ...
  const recentItems: RecentItem[] = [
    ...scholarshipApps.map((a) => ({ id: a.id, title: a.scholarship.title, status: a.status, date: a.createdAt, module: "المنح",   icon: "🎓", href: "/student/scholarships" })),
    ...trainingEnrollments.map((a) => ({ id: a.id, title: a.training.title, status: a.status, date: a.createdAt, module: "التدريب", icon: "🏢", href: "/student/trainings"    })),
  ...
  const stats = [
    { label: "طلبات المنح",  value: scholarshipApps.length,    icon: "🎓", color: "#875E9E", href: "/student/scholarships" },
    { label: "التدريب",     value: trainingEnrollments.length,  icon: "🏢", color: "#4A8FA0", href: "/student/trainings"    },
    ...
    <div className="space-y-6">
      ...
      >
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            <p className="text-cyan-300 text-xs mb-1">بوابة الطالب — جامعة سليمان الراجحي</p>
            ...
            <p className="text-cyan-400 text-xs mt-0.5">عضو منذ {memberSince}</p>
          </div>
          <div className="text-center bg-white/15 rounded-2xl px-5 py-3 shrink-0">
            ...
            <p className="text-cyan-300 text-xs mt-0.5">نقطة مكتسبة</p>
          </div>
        </div>
        ...
        {pendingCount > 0 && (
          <div className="mt-4 flex items-center justify-between gap-4 bg-white/10 rounded-xl px-4 py-2.5 flex-wrap">
            <p className="text-cyan-100 text-xs">
              ⏳ لديك <span className="font-bold text-white">{pendingCount}</span> طلب قيد المراجعة
            </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span>📋</span>
              <h2 className="font-bold text-gray-800 text-sm">آخر النشاطات</h2>
            </div>
            <Link href="/student/my-applications" className="text-xs text-cyan-600 hover:underline font-medium">عرض الكل</Link>
          </div>
          {recentItems.length === 0 ? (
            <div className="px-5 py-10 text-center">
              <p className="text-3xl mb-2">📭</p>
              <p className="text-sm text-gray-400">لا توجد طلبات حتى الآن</p>
            </div>
          ) : (
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
          ...
          {upcomingActivities.length === 0 && upcomingVolunteers.length === 0 ? (
            <div className="px-5 py-10 text-center">
              <p className="text-gray-400 text-sm">لا توجد فعاليات قريبة</p>
            </div>
          ) : (
            <div className="divide-y divide-gray-50">
              {[
          ...
          <div className="px-5 py-3 bg-gray-50 border-t border-gray-100 flex items-center justify-between">
            <Link href="/activities/catalog" className="text-[10px] font-semibold text-blue-600 hover:underline">الأنشطة ←</Link>
            <Link href="/volunteers/opportunities" className="text-[10px] font-semibold text-orange-600 hover:underline">التطوع ←</Link>
          </div>
        </div>
      ...
      <div className="rounded-2xl px-6 py-5 flex items-center justify-between gap-4 flex-wrap bg-gradient-to-l from-cyan-50 to-teal-50 border border-cyan-200">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="font-bold text-cyan-800 text-sm">استفد من جميع الخدمات المتاحة</p>
