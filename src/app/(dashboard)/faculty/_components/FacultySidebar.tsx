...
export default function FacultySidebar({ userName }: Props) {
  ...
  const NavContent = () => (
    <nav className="flex flex-col h-full">
      <div className="px-5 py-5 border-b border-amber-950 flex items-center shrink-0">
        <Image
      ...
      <div className="flex-1 overflow-y-auto px-3 py-3 space-y-4">
        {navGroups.map((group, gi) => (
          <div key={gi}>
            {group.label && (
              <p className="text-[10px] text-amber-600 uppercase tracking-wider px-2 mb-1.5">{group.label}</p>
            )}
  ...
  return (
    <>
      ...
      <aside className="hidden md:flex flex-col w-60 shrink-0 bg-amber-900 min-h-full">
        <NavContent />
      </aside>
