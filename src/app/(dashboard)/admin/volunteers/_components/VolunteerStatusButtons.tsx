...
import { updateVolunteerAppStatus } from "../_actions";
...
export default function VolunteerStatusButtons({ id, initialStatus }: { id: string; initialStatus: string }) {
  const [status, setStatus] = useState(initialStatus);
  const [pending, startTransition] = useTransition();

  function update(next: "APPROVED" | "REJECTED" | "PENDING") {
    startTransition(async () => {
      const res = await updateVolunteerAppStatus(id, next);
