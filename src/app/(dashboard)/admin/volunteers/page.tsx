...
import ManageOpportunitiesTab from "./_components/ManageOpportunitiesTab";
...
export default async function VolunteersAdminPage({
  ...
  const [applications, counts, opportunities] = await Promise.all([
    tab === "applications"
  ...
  const tabs = [
    ...
    { key: "opportunities", label: "إدارة الفرص",   count: null  },
  ];
  ...
  return (
    <div className="space-y-5">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-gray-900">التطوع</h1>
        <p className="text-gray-500 text-sm mt-0.5">إدارة فرص التطوع وطلبات الطلاب</p>
      </div>
      ...
      <div className="flex gap-0 border-b border-gray-200">
        {tabs.map((t) => (
          <a
            key={t.key}
            href={`/admin/volunteers?tab=${t.key}`}
      ...
      {tab === "applications" && (
        <div className="space-y-4">
          ...
          <div className="flex gap-2 flex-wrap">
            {FILTERS.map((f) => {
              const href = f.value ? `/admin/volunteers?tab=applications&filter=${f.value}` : "/admin/volunteers?tab=applications";
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-100">
                  {applications.length === 0 && (
                    <tr>
                      <td colSpan={6} className="px-5 py-8 text-center text-gray-400">لا توجد طلبات</td>
                    </tr>
                  )}
                  {applications.map((app) => (
                    <tr key={app.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-5 py-3 text-gray-400 text-xs">
                        {app.opportunity.date ? app.opportunity.date.toLocaleDateString("ar-SA") : "—"}
                      </td>
                      <td className="px-5 py-3 text-gray-400 text-xs">
                        {app.createdAt.toLocaleDateString("ar-SA")}
                      </td>
                      <td className="px-5 py-3">
                        <VolunteerStatusButtons id={app.id} initialStatus={app.status} />
                      </td>
                    </tr>
                  ))}
                </tbody>
      ...
      {tab === "opportunities" && (
        <ManageOpportunitiesTab opportunities={opportunities as any} />
      )}
    </div>
  );
}
