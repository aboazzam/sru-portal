...
import ManageScholarshipsTab from "./_components/ManageScholarshipsTab";
...
export default async function ScholarshipsAdminPage({
  ...
  const [applications, counts, scholarships] = await Promise.all([
    tab === "applications"
  ...
  const tabs = [
    ...
    { key: "scholarships", label: "إدارة المنح", count: null  },
  ];
  ...
  return (
    <div className="space-y-5">
      ...
      <div>
        ...
        <p className="text-gray-500 text-sm mt-0.5">إدارة المنح وطلبات الطلاب</p>
      </div>
      ...
      <div className="flex gap-0 border-b border-gray-200">
        {tabs.map((t) => (
          <a
            key={t.key}
            href={`/admin/scholarships?tab=${t.key}`}
      ...
      {tab === "applications" && (
        <div className="space-y-4">
          ...
          <div className="flex gap-2 flex-wrap">
            {FILTERS.map((f) => {
              const href = f.value ? `/admin/scholarships?tab=applications&filter=${f.value}` : "/admin/scholarships?tab=applications";
      ...
      {tab === "scholarships" && (
        <ManageScholarshipsTab scholarships={scholarships as any} />
      )}
    </div>
  );
}
