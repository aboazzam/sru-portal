...
export default async function ActivitiesAdminPage({
  ...
  const [applications, counts, activities] = await Promise.all([
    tab === "applications"
  ...
  return (
    <div className="space-y-5">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-gray-900">الأنشطة الطلابية</h1>
        <p className="text-gray-500 text-sm mt-0.5">إدارة الأنشطة وطلبات المشاركة</p>
      </div>
      ...
      <div className="flex gap-0 border-b border-gray-200">
        {tabs.map((t) => (
          <a
            key={t.key}
            href={`/admin/activities?tab=${t.key}`}
      ...
      {tab === "applications" && (
        <div className="space-y-4">
          ...
          <div className="flex gap-2 flex-wrap">
            {FILTERS.map((f) => {
              const href = f.value ? `/admin/activities?tab=applications&filter=${f.value}` : "/admin/activities?tab=applications";
                  ...
                  {applications.map((app) => (
                    <tr key={app.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-5 py-3 text-gray-400 text-xs">
                        {app.activity.date ? app.activity.date.toLocaleDateString("ar-SA") : "—"}
                      </td>
                      <td className="px-5 py-3 text-gray-400 text-xs">
                        {app.createdAt.toLocaleDateString("ar-SA")}
                      </td>
                      <td className="px-5 py-3">
                        <ActivityStatusButtons id={app.id} initialStatus={app.status} />
                      </td>
                    </tr>
                  ))}
                </tbody>
      ...
      {tab === "activities" && (
        <ManageActivitiesTab activities={activities as any} />
      )}
    </div>
  );
}
