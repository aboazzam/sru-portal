...
import { applicationJourney } from "@/lib/mock/scholarships";
...
export default function ApplyForm({ scholarships, appliedIds }: Props) {
  ...
  const available  = scholarships.filter((s) => !appliedSet.has(s.id));
  ...
  function submit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const result = await submitScholarshipApplication(selected);
      if (result.success) {
        setSubmitted(true);
      } else {
  ...
  return (
    <div className="space-y-6">
          ...
          {[
            ...
            return (
              <div key={s.id} className="flex items-center flex-1">
                ...
                {i < 3 && (
                  <div className="flex-1 h-0.5 mx-2" style={{ background: done ? "#875E9E" : "#E8EDEF" }} />
                )}
              </div>
            );
          })}
      ...
      {/* Error banner */}
      {error && (
        <div className="flex items-center gap-3 px-5 py-4 rounded-xl" style={{ background: "#FEE2E2", color: "#DC2626", border: "1px solid #FCA5A5" }}>
          <span>❌</span>
      ...
      <form onSubmit={submit} className="space-y-4">
          ...
          <div className="bg-white rounded-2xl border border-[#E8EDEF] p-5 space-y-4">
            ...
            {available.length === 0 && (
              <div className="text-center py-8">
                <p className="text-[#8FA4AB]">
                  {scholarships.length === 0
                    ? "لا توجد منح متاحة حالياً"
                    : "لقد تقدّمت على جميع المنح المتاحة"}
                </p>
                <Link
                  href="/scholarships/track"
            ...
            {available.length > 0 && (
              <>
                <div className="space-y-2 max-h-80 overflow-y-auto">
                  {available.map((s) => {
                    const isPartial = s.title.includes("%");
                    const color = isPartial ? "#875E9E" : "#6CAEBD";
                    return (
                      <label
                        key={s.id}
                        ...
                        style={selected === s.id ? { borderColor: color, background: color + "0A" } : undefined}
                      >
                        <input
                          type="radio"
                          name="scholarship"
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="font-semibold text-sm text-[#1F2937] leading-snug">{s.title}</p>
                          {s.description && (
                            <p className="text-xs text-[#8FA4AB] mt-0.5 line-clamp-2">{s.description}</p>
                          )}
                          <span
                            className="inline-block mt-1 text-xs px-2 py-0.5 rounded-full font-medium"
                            style={{ background: color + "18", color }}
                          >
                            منحة {isPartial ? "جزئية" : "كاملة"}
                          </span>
                        </div>
                      </label>
                    );
        ...
        {step === "docs" && (
          <div className="bg-white rounded-2xl border border-[#E8EDEF] p-5 space-y-4">
            ...
            <p className="text-xs text-[#6B7280]">يرجى رفع المستندات المطلوبة لإتمام التقديم.</p>
            {[
        ...
        {step === "confirm" && (
          <div className="bg-white rounded-2xl border border-[#E8EDEF] p-5 space-y-4">
            ...
            <div className="rounded-xl border border-[#E8EDEF] divide-y divide-[#F4F7F8]">
              {[
                { label: "نوع المنحة",   value: scholarships.find((s) => s.id === selected)?.title ?? "—" },
