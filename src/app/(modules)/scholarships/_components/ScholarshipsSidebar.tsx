...
const navItems = [
  { href: "/scholarships",         label: "نظرة عامة",          icon: "🏠" },
  { href: "/scholarships/full",    label: "المنح الكاملة",       icon: "🎓" },
  { href: "/scholarships/partial", label: "المنح الجزئية",       icon: "📊" },
  { href: "/scholarships/funding", label: "الحلول التمويلية",    icon: "💡" },
  { href: "/scholarships/apply",   label: "تقديم طلب",          icon: "📝" },
  { href: "/scholarships/track",   label: "تتبع الطلب",         icon: "🔍" },
];

interface Props { userName: string; userRole: string }

export default function ScholarshipsSidebar({ userName, userRole }: Props) {
    ...
    <nav className="flex flex-col h-full">
      ...
      <div className="px-5 py-3 border-b border-[#E8EDEF]">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-bold" style={{ color: "#875E9E" }}>المنح والحلول المالية</span>
        </div>
      </div>
              ...
              <Link
                ...
                className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all duration-150 ${
                  isActive
                    ...
                    : "text-[#506570] hover:bg-[#F3EEF7] hover:text-[#6A4A7E]"
                }`}
                style={isActive ? { background: "#875E9E" } : undefined}
              >
