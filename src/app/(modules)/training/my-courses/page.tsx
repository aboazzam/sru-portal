...
const trainingStatusConfig: Record<string, { label: string; color: string; bg: string }> = {
  UPCOMING:  { label: "قادم", color: "#2563EB", bg: "#DBEAFE" },
  ONGOING:   { label: "جارٍ", color: "#16A34A", bg: "#DCFCE7" },
  COMPLETED: { label: "منتهٍ", color: "#6B7280", bg: "#F3F4F6" },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-2xl font-bold text-[#1A2A30]">دوراتي</h1>
          <p className="text-[#8FA4AB] text-sm mt-0.5">
            {active.length} نشطة · {completed.length} مكتملة · {enrollments.length} إجمالي
          </p>
        </div>
        <Link
      ...
      {enrollments.length === 0 && (
        <div className="bg-white rounded-2xl border border-[#E8EDEF] py-16 text-center">
          ...
          <h3 className="font-bold text-[#1A2A30] text-base">لم تسجّل في أي برنامج بعد</h3>
          <p className="text-[#8FA4AB] text-sm mt-1 mb-4">تصفّح البرامج المتاحة وابدأ رحلتك التدريبية</p>
          <Link
            href="/training/catalog"
      ...
      {active.length > 0 && (
        <div className="bg-white rounded-2xl shadow-sm border border-[#E8EDEF] overflow-hidden">
          <div className="px-5 py-4 border-b border-[#E8EDEF] flex items-center gap-2">
            <span>📖</span>
            <h2 className="font-bold text-[#1A2A30] text-sm">الجارية والمسجّلة</h2>
                  ...
                  <div className="flex items-start justify-between gap-3 flex-wrap">
                    ...
                    <div className="shrink-0 text-end text-xs text-[#9CA3AF]">
                      {enr.training.startDate && (
                        <p>البدء: {new Date(enr.training.startDate).toLocaleDateString("ar-SA")}</p>
                      )}
                      {enr.training.endDate && (
                        <p>الانتهاء: {new Date(enr.training.endDate).toLocaleDateString("ar-SA")}</p>
                      )}
                    </div>
                  </div>
                </div>
      ...
      {completed.length > 0 && (
        <div className="bg-white rounded-2xl shadow-sm border border-[#E8EDEF] overflow-hidden">
          <div className="px-5 py-4 border-b border-[#E8EDEF] flex items-center gap-2">
            <span>🏁</span>
            <h2 className="font-bold text-[#1A2A30] text-sm">المكتملة</h2>
