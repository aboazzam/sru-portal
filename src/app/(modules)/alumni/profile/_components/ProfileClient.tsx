...
export default function ProfileClient({ name, email }: Props) {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-[#E8EDEF] shadow-sm p-5">
        <div className="flex items-center justify-between gap-3 flex-wrap mb-4">
          ...
          <span className="text-sm font-bold px-3 py-1 rounded-full" style={{ background: "#F3EEF7", color: "#875E9E" }}>
            {pct}%
          </span>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          {profileCompletionItems.map((item) => (
            <div key={item.id} className="flex items-center gap-3 p-2.5 rounded-xl" style={{ background: "#F4F7F8" }}>
              <span className={`text-base shrink-0 ${item.done ? "" : "opacity-40"}`}>{item.done ? "✅" : "⭕"}</span>
              <span className={`text-xs font-medium flex-1 ${item.done ? "text-[#1F2937]" : "text-[#9CA3AF]"}`}>{item.label}</span>
              <span className="text-xs font-bold shrink-0" style={{ color: item.done ? "#16A34A" : "#D97706" }}>
                +{item.points}%
              </span>
            </div>
          ))}
        ...
        {missing.length > 0 && (
          <div className="mt-4 px-4 py-3 rounded-xl flex items-start gap-3" style={{ background: "#FEF3C7", border: "1px solid #FCD34D" }}>
      ...
      <div className="bg-white rounded-2xl border border-[#E8EDEF] shadow-sm overflow-hidden">
        <div className="px-5 py-3 flex items-center justify-between gap-2" style={{ background: "#F4F7F8", borderBottom: "1px solid #E8EDEF" }}>
          <div className="flex items-center gap-2">
            <span>🎓</span>
            <h2 className="font-bold text-[#1A2A30] text-sm">بيانات التخرج</h2>
          </div>
          <span className="flex items-center gap-1 text-xs font-semibold px-2.5 py-1 rounded-full" style={{ background: "#E8EDEF", color: "#6B7280" }}>
            🔒 غير قابلة للتعديل
          </span>
        </div>
        <div className="divide-y divide-[#F4F7F8]">
          {[
            { label: "الاسم الكامل (عربي)",    value: name },
      ...
      <div className="bg-white rounded-2xl border border-[#E8EDEF] shadow-sm overflow-hidden">
        <div className="px-5 py-3 flex items-center gap-2" style={{ background: "linear-gradient(to left, #875E9E08, #6CAEBD08)", borderBottom: "1px solid #E8EDEF" }}>
          <span>✏️</span>
        ...
        <form onSubmit={handleSave} className="p-5 space-y-5">
          ...
          <div className="flex items-center gap-3 pt-2">
            ...
            {saved && (
              <span className="text-xs font-semibold px-3 py-1.5 rounded-full animate-pulse" style={{ background: "#DCFCE7", color: "#16A34A" }}>
                ✅ تم الحفظ بنجاح
              </span>
            )}
          </div>
      ...
      <div className="bg-white rounded-2xl border border-[#E8EDEF] shadow-sm overflow-hidden">
        <div className="px-5 py-3 flex items-center gap-2" style={{ borderBottom: "1px solid #E8EDEF" }}>
          <span>🏆</span>
          <h2 className="font-bold text-[#1A2A30] text-sm">الإنجازات والشارات</h2>
          <span className="ms-auto text-xs font-semibold px-2.5 py-1 rounded-full" style={{ background: "#F3EEF7", color: "#875E9E" }}>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    <span className="text-xs font-semibold px-2 py-0.5 rounded-full" style={{ background: "#fff", color: cat.color }}>
                      {cat.label}
                    </span>
